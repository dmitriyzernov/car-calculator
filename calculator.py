...
class Calculator:
    def __init__(self, mileage=15000, years=3, year_loss=10):
        ...
        self.cars = {}  # dictionary будет содержать пары машина и ее годовая стоимость, т.е. Car: Year Price
        self.years = years
...
class Car:
    ...
    def static_year_cost(self):
        cost = self.service_cost + self.insurance_cost
        return cost

    #        return self.service_cost + self.insurance_cost    // можно было так написать строки 19 и 20
...
def add_two_numbers_2(a, b):
    if b == 0:
        return 0
    return round(a/b, 2) # в скобках цифра 2 означет сколько цифр будет после запятой
